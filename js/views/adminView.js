...
const renderCreateForm = () => {
    ...
    panel.querySelector('form').addEventListener('submit', async event => {
        ...
        try { await LicenciaRepo.crear({ codigo: new FormData(form).get('codigo'), cliente: new FormData(form).get('cliente'), email: new FormData(form).get('email'), cupoTotal: new FormData(form).get('creditos') }); panel.innerHTML = ''; await loadLicenses(); }
...
const renderLicenses = licenses => {
    ...
    container.querySelectorAll('.edit-license-form').forEach(form => form.addEventListener('submit', async event => {
        ...
        try { await LicenciaRepo.actualizar(form.dataset.id, { cliente: values.get('cliente'), email: values.get('email'), activa: values.get('activa') === 'on' }); await loadLicenses(); } catch (exception) { error.textContent = exception.message; }
    }));
};
